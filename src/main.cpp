...
int main(int argc, const char *argv[]) {
    ...
    cout << ast->koopa_ir() << endl;

    auto koopa_ir_str = ast->koopa_ir();

    // 解析字符串 str, 得到 Koopa IR 程序
    koopa_program_t program;
    koopa_error_code_t ret1 = koopa_parse_from_string(koopa_ir_str.c_str(), &program);
    ...
    auto funcs = raw.funcs;
    auto buf = funcs.buffer;
    auto len = funcs.len;
    auto kind = funcs.kind;


    for (int i = 0; i < len; i++) {
        auto func = (koopa_raw_function_t) buf[i];
        cout << func -> name << endl;
    }
