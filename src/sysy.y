...
// 非终结符的类型定义
%type <str_val> FuncDef FuncType Block Stmt Number VarType
...
Block
  : '{' Stmt Stmt '}' {
    auto stmt1 = unique_ptr<string>($2);
    auto stmt2 = unique_ptr<string>($3);
    $$ = new string("{ " + *stmt1 + *stmt2 + " }");
  }
  ;
...
Stmt
  ...
  VarType IDENT ';' {
  auto type = unique_ptr<string>($1);
  auto iden = unique_ptr<string>($2);
    $$ = new string(*type + " " +  *iden + ";");
  }
  ;
